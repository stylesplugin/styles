...
jQuery(function($) {
	...
	$('.stop-markers .marker').unbind('click').dblclick( handle_color_picker );
	$('input.grad_color').change( combine_stops );
	...
	function update_handle_color( ibox ) {
		...
		$( ibox ).parents('tr.pds_gradient').find('div.stop-markers > div.marker:eq('+index+') a')
			.css({
				'backgroundColor': '#' + hex
			});
	}
	...
	function handle_color_picker( ) {
		...
		$( this ).parents('tr.pds_gradient').find('div.colors > input').eq(index).click();
		
		return false;
	}
	
	function combine_stops( el ){
		var wrapper, 
			stops = new Array();
		
		wrapper = $(this).parents('tr.pds_gradient');
		if (undefined == wrapper.context ) { wrapper = $(el).parents('tr.pds_gradient'); }
		
		$(wrapper).find('div.stop-markers a').each(function(){
			var percent = $(this).parent().slider('value');
			var color = rgbtohex( $(this).css('backgroundColor') );
			stops.push( color+' '+percent+'%' );
		});
		
		wrapper.find('input.stops').val(stops);
		wrapper.find('div.preview').css('background', '-moz-linear-gradient(0deg, '+stops+')');
	...
	combine_stops($('input.grad_color'));
...
// remember to change every instance of "gradientPicker" to the name of your plugin!
(function($) {
