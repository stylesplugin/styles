...
(function($) {
    ...
    $.gradientPicker = function(element, options) {
		...
		var sliderOpts = {
			...
			max: 100,
			value: 0,
			stop: function(){ $element.find('input.stops').change() },
			slide: function (){ plugin.setGradientStops(); }
		};
		...
		plugin.init = function() {
			...
			// Gradient Slider
			$markers = $element.find('div.stop-markers .marker');
			$markers.eq(0).slider( sliderOpts );
			sliderOpts.value = 100;
			$markers.eq(1).slider( sliderOpts );
			$markers.unbind('click');
			
			// Attach ColorPicker to slider handles
			$markers.ColorPicker( colorPickerOpts )

			plugin.getGradientStops();
			plugin.setGradientStops();
        }
		...
		plugin.getGradientStops = function () {
			...
			$markers.each(function(){
				...
				setHandleColor( this, tmp[i][0]);
				$(this).slider('value', tmp[i][1] );
			});
		}
		
		plugin.setGradientStops = function() {
			stops = new Array();
			...
			if ( stops != $element.find('input.stops').val() ) {
				$element.find('input.stops').val(stops).change();
			}
			$element.find('div.grad-preview').css('background', '-moz-linear-gradient(0deg, '+stops+')');
